...
long long solve(int ax, int ay, int bx, int by, int px, int py, bool part2) {
  ...
  if (best_score == numeric_limits<long long>::max())
    return 0;

  long long amt = (P2 - 40000) / best_dx;
  ...
  function<long long(long long, long long)> f = [&](long long x, long long y) {
    ...
    long long ans = min(3LL + f(x - ax, y - ay), 1LL + f(x - bx, y - by));

    DP[{x, y}] = ans;
    return ans;
  };

  long long ans = f(px + P2 - amt * best_dx, py + P2 - amt * best_dx);

  if (ans < 1000000000000000LL) {
    return ans + amt * best_cost;
  }
...
int main(int argc, char *argv[]) {
  ...
  while ((pos = D.find("\n\n")) != string::npos) {
    ...
    // Parse A button coordinates
    int ax = stoi(a.substr(a.find("+") + 1));
    int ay = stoi(a.substr(a.rfind("+") + 1));

    // Parse B button coordinates
    int bx = stoi(b.substr(b.find("+") + 1));
    int by = stoi(b.substr(b.rfind("+") + 1));

    // Parse prize coordinates
    int px = stoi(prize.substr(prize.find("=") + 1));
    int py = stoi(prize.substr(prize.rfind("=") + 1));
