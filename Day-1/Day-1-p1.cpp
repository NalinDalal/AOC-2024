...
// sort both the list
// then take difference b/w the ith member of both list
// total distance = sum of all the differences
/*
#include <algorithm>
#include <iostream>
#include <numeric>
...
int main() {
  // Open the file
  ifstream file("Day-1-p1-data.txt");
  if (!file.is_open()) {
    cerr << "Error opening file!" << endl;
    return 1;
  }

  // Read pairs into vectors
  ...
  if (left.size() != right.size()) {
    cerr << "Mismatched number of left and right values!" << endl;
    return 1;
  }

  int n = left.size(); // Number of pairs
  cout << "Number of pairs: " << n << endl;

  // Output the left and right lists
  cout << "Left list members: ";
  for (int val : left) {
    cout << val << " ";
  }
  cout << endl;

  cout << "Right list members: ";
  for (int val : right) {
    cout << val << " ";
  }
  cout << endl;
  ...
  for (int val : left) {
    if (right_counts.find(val) != right_counts.end()) {
      similarity_score += val * right_counts[val];
    }
  }
