...
// check which of the order is correct as per the rules
// collect those are correct, then
// add up the middle numbers i.e. from nums[1] to nums[n-1]
// return the sum
...
int main() {
  ...
  // Check validity of updates and calculate the sum of middle pages
  int sum_of_middle_pages = 0;
  for (const auto &update : updates) {
    if (is_valid_update(update, rules)) {
      int middle_index = update.size() / 2; // Middle page (0-indexed)
      sum_of_middle_pages += update[middle_index];
    }
  }

  // Output the result
  cout << sum_of_middle_pages << endl;
  return 0;
}
